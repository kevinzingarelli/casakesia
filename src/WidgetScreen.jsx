...
export default function WidgetScreen({ data, choresById, totals, streaks, me, t, dark, health }) {
  ...
  const SmallWidget = ({ user }) => {
    ...
    return (
      <div style={{ background: gradient, borderRadius: '20px', padding: '16px', width: '160px', height: '160px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: cardShadow }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: '26px' }}>{user.emoji}</span>
          <span style={{ fontSize: '20px' }}>{lvl.emoji}</span>
        </div>
        <div>
          <div className="display" style={{ fontSize: '34px', fontWeight: 800, color: '#fff', lineHeight: 1 }}>{totals[user.id] || 0}</div>
  ...
  const MediumWidget = () => (
    <div style={{ background: t.card, borderRadius: '20px', padding: '16px', width: '340px', height: '160px', boxShadow: cardShadow, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        return (
          <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: '8px', background: t.line, borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${pct}%`, background: u.color, borderRadius: '4px', transition: 'width 0.5s' }} />
              </div>
            </div>
          </div>
        );
  ...
  // WIDGET GRANDE: tutto
  const LargeWidget = () => (
    <div style={{ background: t.card, borderRadius: '20px', padding: '18px', width: '340px', boxShadow: cardShadow }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <span className="display" style={{ fontSize: '15px', fontWeight: 800, color: t.text }}>🏠 Casa Points</span>
      ...
      <div style={{ background: gradient, borderRadius: '16px', padding: '12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
  ...
  return (
    <div className="fade-in" style={{ padding: '0 18px' }}>
      <div style={{ background: dark ? 'rgba(255,209,102,0.12)' : '#FFF9E6', borderRadius: '16px', padding: '12px 14px', marginBottom: '16px', fontSize: '12px', color: t.text, lineHeight: 1.5 }}>
